  ...
  setup_all do
    on_exit fn ->
     ...
     Enum.each(notifications, fn key -> Riak.find("test-notifications", "test-notifications", key) end)
       ...
       Enum.each(keys, fn key -> Riak.delete("test-notification-users", "test-notification-users", key) end)
       Enum.each(keys, fn key -> Riak.find("test-notifications-users", "test-notification_users", key) end)
